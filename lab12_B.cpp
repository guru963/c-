    ...
    void deleteEdge(int start, int end) {
        if (isValidVertex(start) && isValidVertex(end)) {
            if (isEdgePresent(start, end)) {
                ...
                adjList[end].remove(start); // For undirected graph
...
private:
    ...
    bool isEdgePresent(int start, int end) {
        return find(adjList[start].begin(), adjList[start].end(), end) != adjList[start].end();
    }
};
